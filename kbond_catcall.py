...
SECTOR_CAT = [
    ...
    # ★[OWNER 2026-09-09] 「캐」 홑말은 캐피탈이다 — 「1년이내 캐 사자」·「25.8~10월 캐 -4원 사자」
    ...
    # ★[OWNER 2026-09-09] 「당사」 = 지표물 사자. 「국통당사」 = 국고·통안 지표물.
    ("국고",      re.compile(r'(?<![가-힣])국고(?![가-힣])|국채|국통당사|(?<![가-힣])당사(?![가-힣])')),
    ("통안",      re.compile(r'통안|국통당사')),
...
# ★[OWNER 2026-09-09] 「1.5년 언저리」 = 잔존 1.5년 가량.
RE_T_ABOUT   = re.compile(r'(?:잔존\s*)?' + _N + r'\s*년\s*(?:내외|정도|언저리|가량|쯤|짜리|물|물건)?')
# ★[OWNER 2026-09-09] 「장기 공사채」 = 10년 이상. 표현 자체가 만기대를 말한다.
# ⚠ 뒤에 범주어가 붙어 온다 — 「장기공사채」·「장기물」. 꼬리 제약을 두면 안 걸린다.
RE_T_LONG    = re.compile(r'(?<![가-힣])장기(?!간|적)')
...
RE_M_YQ  = re.compile(r'(\d{2})\s*년\s*(초중반|초중|초|중|말)')
# 「25년 말~ 26년 초」 처럼 두 어림수를 잇는 꼴
RE_M_YQ2 = re.compile(r'(\d{2})\s*년\s*(초|중|말)\s*[~∼–—-]\s*(\d{2})\s*년\s*(초|중|말)')

# ★[OWNER 2026-09-09] 「27년 초중반」 = 27년 1~2분기.
QUARTER = {"초": (1, 3), "중": (5, 8), "말": (10, 12), "초중반": (1, 6), "초중": (1, 6)}
...
def _tenor_direct(text: str):
    ...
    if m:
        ...
        return v, v
    if RE_T_LONG.search(text):
        return 10.0, np.nan          # [OWNER] 장기 = 10년 이상
